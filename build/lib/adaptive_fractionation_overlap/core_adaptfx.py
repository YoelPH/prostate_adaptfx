...
def adaptive_fractionation_core(fraction, volumes, accumulated_dose, number_of_fractions = 5, min_dose = 7.25, max_dose = 9.25, mean_dose = 8, alpha = 4.476900395372424, beta = 0.10723772034080617):
    ...
    if all(x == volumes[0] for x in volumes) == True: #if all sparing factors are identical we just deliver the mean dose. Can be changed to a very small std in theory (then we deliver min_dose)
        ...
        for state, fraction_state in enumerate(np.arange(number_of_fractions, fraction-1, -1)):
            ...
            if (state == number_of_fractions - 1):  # first fraction with no prior dose delivered so we dont loop through dose_space
                overlap_penalty = (actual_volume * (delivered_doses - min_dose).clip(0)).clip(0) #This means only values over min_dose get a penalty. Values below min_dose do not get a reward
                ...
                Vs = -overlap_penalty + future_values
                actual_policy = delivered_doses[Vs.argmax()]
                ...
                overlap_penalty = (actual_volume * (delivered_doses_clipped - min_dose).clip(0)).clip(0) #This means only values over min_dose get a penalty. Values below min_dose do not get a reward
    ...
    # policies = policies *0.05+min_dose
    return [policies, volume_space, physical_dose, penalty_added, Values, dose_space]
...
def adaptfx_full(volumes, number_of_fractions = 5, min_dose = 7.25, max_dose = 9.25, mean_dose = 8):
    ...
    for index, frac in enumerate(range(1,6)):
        if frac != number_of_fractions:
            [policies, volume_space, physical_dose, penalty_added, Values, dose_space]  = adaptive_fractionation_core(frac,volumes[:-number_of_fractions+frac],accumulated_doses[index], min_dose = min_dose, max_dose = max_dose, mean_dose = mean_dose)
            accumulated_doses[index+1] = accumulated_doses[index] + physical_dose
        else:
            [policies, volume_space, physical_dose, penalty_added, Values, dose_space]  = adaptive_fractionation_core(frac,volumes,accumulated_doses[index], min_dose = min_dose, max_dose = max_dose, mean_dose = mean_dose)
        physical_doses[index] = physical_dose
    return physical_doses, accumulated_doses
