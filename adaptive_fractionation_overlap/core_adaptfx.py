...
def adaptive_fractionation_core(fraction: int, volumes: np.ndarray, accumulated_dose: float, number_of_fractions: int = 5, min_dose: float = 7.25, max_dose: float = 9.25, mean_dose:float  = 8, alpha: float = 1.8380125313579265, beta:float = 0.2654168553532238):
...
def adaptfx_full(volumes: list, number_of_fractions: float = 5, min_dose: float = 7.25, max_dose: float = 9.25, mean_dose: float = 8):
    ...
    for index, frac in enumerate(range(1,6)):
        if frac != number_of_fractions:
            [policies, policies_overlap, volume_space, physical_dose, penalty_added, values, dose_space, probabilities, final_penalty]  = adaptive_fractionation_core(frac,volumes[:-number_of_fractions+frac],accumulated_doses[index], min_dose = min_dose, max_dose = max_dose, mean_dose = mean_dose)
            accumulated_doses[index+1] = accumulated_doses[index] + physical_dose
        else:
            [policies, policies_overlap, volume_space, physical_dose, penalty_added, values, dose_space, probabilities, final_penalty]  = adaptive_fractionation_core(frac,volumes,accumulated_doses[index], min_dose = min_dose, max_dose = max_dose, mean_dose = mean_dose)
...
def precompute_plan(fraction: int, volumes: np.ndarray, accumulated_dose: float, number_of_fractions: int = 5, min_dose: float = 7.25, max_dose: float = 9.25, mean_dose:float  = 8, alpha: float = 1.8380125313579265, beta:float = 0.2654168553532238):
    ...
    volumes_to_check = np.arange(0,6.6,0.1)
    predicted_policies = np.zeros(len(volumes_to_check))
    for index, volume in enumerate(volumes_to_check):
        [policies, policies_overlap, volume_space, physical_dose, penalty_added, values, dose_space, probabilities, final_penalty] = adaptive_fractionation_core(fraction, np.append(volumes,volume), accumulated_dose, number_of_fractions, min_dose, max_dose, mean_dose, alpha, beta)
